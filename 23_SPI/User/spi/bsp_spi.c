...
uint8_t SPI_Power_Down(void){
	...
	SPI_CS_HIGH();
	SPI_delay(0x1000);
	SPI_Debuy_SUCCESS |= SPI_POWER_DOWN_FLAG;
	return SPI_POWER_DOWN_FLAG;
}
...
uint8_t SPI_SPI_Sector_Erase(uint32_t address){
	uint8_t temp;
	WriteEnable();
	SPI_CS_LOW();
	ReadByte(0x20);
...
static uint8_t SPI_Write_Page(uint8_t* data, uint32_t address, uint32_t size){
	if(size>256) return 0;//size not access
	
	//SPI_SPI_Sector_Erase(address);
	// must be enable before execution
	WriteEnable();
	SPI_CS_LOW();
	ReadByte(0x02);
	ReadByte((address&0xFF0000)>>16);
...
uint8_t SPI_SectorWrite(uint8_t* data, uint32_t address, uint32_t size){
	uint8_t i;
	uint8_t page_need = size/256;
	uint8_t remains = size%256;
	
	SPI_SPI_Sector_Erase(address);
	for(i=0;i<page_need;i++){
		SPI_Write_Page(&data[i*256],address,256);
		address += 256;
	}
	
	if(remains!=0){
		SPI_Write_Page(&data[i*256],address,remains);
	}
	return SPI_Debuy_SUCCESS;
}
...
//maximum support 4GB
	uint16_t i;
	uint16_t sectors_need = size/4096;
	uint16_t byte_remains = size%4096;
	if(sectors_need!=0){
		for(i=0;i<sectors_need;i++){
			SPI_SectorWrite(&data[i*4096], address, 4096);
			address+=4096;
		}
	}
	//write remaining bytes
	SPI_Write_Page(&data[i*4096], address, byte_remains);
	return SPI_Debuy_SUCCESS;
}
...
uint8_t Chip_Erase(uint8_t enable){
	...
	if(enable == 1){
		...
		ReadByte(0xC7);
		SPI_CS_HIGH();
		SendString("NOR Flash is formatting, please wait...\n");
		SPI_CS_LOW();
		ReadByte(0x05);
		do{
			state = ReadByte(Dummy);
		...
		SPI_CS_HIGH();
		SendString("Success! NOR Flash is formatted!\n");
	}
	return SPI_Debuy_SUCCESS;
}
